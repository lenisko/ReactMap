...
export function Settings() {
  ...
  const poracleFollowMeValue = useStorage((s) => s.poracleFollowMe)

  /** @type {import('@mui/material').SwitchProps['onChange']} */
  const handleFollowMeChange = React.useCallback(
    (_, checked) => {
      // Show warning dialog when toggling from OFF to ON
      if (checked && !poracleFollowMeValue) {
        setShowFollowMeWarning(true)
      }
    },
    [poracleFollowMeValue],
  )
